# BioMaster 全方向疾病推理恢复与新版图谱增量审计（2026-09-09）
...
来源为[Every Cure公开数据](https://docs.dev.everycure.org/releases/public_data_releases/)。固定`kg-edges`修订 `fcd0585957ab5137771b256dad3714dcaf44b7db`；各数据集独立修订和发布者SHA256见 `eckg/SOURCE_MANIFEST.json`。本轮扫描全部8个节点分片、49个关系分片：7,354,612节点、81,149,812关系。与项目实体相连的原始关系8,305,253条，完整限定条件及来源保存在Parquet。

筛出药—疾病、项目药—项目靶点、靶点—疾病，按选定关系/方向/来源字段去重后为855,294条审计记录。这不是855,294个新发现，也不是全部图边的唯一性声明。

再要求：旧图端点可比较、旧图没有该端点配对、存在非文本来源断言、项目节点映射唯一、无已发现身份冲突；得到 **45,751个待核验的新增端点关系**：
...
另外12个候选对在EC出现非文本、无节点多义的既有来源关系，详见 `SPR512_ECKG_RELATION_REVIEW.csv`：

"""Inference for a compact, self-contained old-drug/target catalog bundle."""
...
import pandas as pd
import torch
from .unified_interaction import UnifiedInteraction,UnifiedConfig
from .refined_interaction import RefinedInteraction,RefinedConfig
...
class CatalogRanker:
    ...
    def __init__(self,directory,device='cpu',verify=True):
        ...
        if self.metadata['format_version']!=1:raise ValueError('unsupported bundle format')
        ...
        if state['architecture']=='unified':self.model=UnifiedInteraction(UnifiedConfig(**state['config']))
        elif state['architecture']=='refined':self.model=RefinedInteraction(RefinedConfig(**state['config']))
        else:raise ValueError('unknown packaged architecture')
        ...
        expected={'drug_global':(nd,512),'drug_graph_mean':(nd,40),'pretrained_available':(nd,),'target_global':(nt,1280)}
    ...
    def rank_targets(self,drug_id,candidates=None,top_k=20,**kwargs):
        ...
        return self._rank(self.score_pairs([drug_id]*len(candidates),candidates,**kwargs),
                          'target_id','drug_to_target_score',top_k)
    ...
    def rank_drugs(self,target_id,candidates=None,top_k=20,**kwargs):
        ...
        return self._rank(self.score_pairs(candidates,[target_id]*len(candidates),**kwargs),
                          'drug_id','target_to_drug_score',top_k)

...
export default function FrontierModels({ kind, identifier, compact = false }: {kind?: string; identifier?: string; compact?: boolean}) {
  ...
  useEffect(() => setPage(1),[query,sort,kind,identifier,commonOnly]);
  const rows = [...(data?.items || [])].filter(r => (!commonOnly || r.disagreement_span!=null) && `${r.candidate_id} ${r.drug_name} ${r.gene} ${r.drug_id} ${r.target_id}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <section className="panel frontier-models">
    ...
    <p>原384候选上的 Nesso-1、ProbeMatchDTI、DTBind 复核。查看不同模型是否给出一致的排序判断，并在真实亲和标签上比较。</p>
    ...
    {!data ? <p role="status">正在读取模型结果…</p> : !data.available ? <p>本轮结果尚未生成。</p> : <>
      ...
      <div className="frontier-coverage">{data.models.map(m => <article key={m.id}><strong>{m.name}</strong><span>{stateText[m.state] || m.state}</span><b>{m.spr_scored}<small> / 384 候选</small></b><span>有标签测试 {m.benchmark_scored} / 479</span>{(m.failed>0 || m.unavailable>0) && <small>候选：{m.spr_unavailable || 0} 未覆盖 · {m.spr_failed || 0} 失败</small>}<a href={data.source_urls[m.id]} target="_blank" rel="noreferrer">官方来源 ↗</a></article>)}</div>
      ...
      {view==="plot" ? <div className="frontier-plot"><p>当前页 {plotted.length} 条轨迹 · 共同池 {data.common_spr_pairs} 对。越靠上表示排名越靠前；悬停或用Tab选择候选。</p><div className="frontier-scroll"><svg viewBox="0 0 1320 370" role="group" aria-label="原384候选七模型共同样本排名轨迹">
        ...
        {data.model_order.map((m,i)=><g key={m}><line x1={145+i*180} x2={145+i*180} y1="65" y2="310" stroke="#a3b7d1"/><text x={145+i*180} y="30" textAnchor="middle" fontSize="14" fontWeight="600" fill="#253953">{data.model_names[m].replace(/（.*）/g,"").replace(" · 结合预测","")}</text></g>)}
      ...
      </svg></div><div className="frontier-inspector" aria-live="polite">{active?<><strong>{active.candidate_id} · {active.drug_name} → {active.gene}</strong><span>{data.model_order.map(m=>`${data.model_names[m]} #${fmt(active.common_ranks[m],1)}`).join(" · ")}</span></>:<span>选择一条轨迹查看每个模型的具体排名。</span>}</div></div> : <div className="frontier-scroll"><table className="frontier-pairs"><thead><tr><th>原候选 / 药物 → 靶点</th>{data.model_order.map(m=><th key={m}>{data.model_names[m]}<small>共同候选池内排名</small></th>)}<th>分位跨度</th></tr></thead><tbody>{visible.map(r=><tr key={r.pair_id}><th><small>{r.candidate_id} · 原顺序 {r.priority}</small><a href={`#/drug/${r.drug_id}/rankings`}>{r.drug_name}</a><a href={`#/target/${r.target_id}/rankings`}>→ {r.gene}</a></th>{data.model_order.map(m=><td key={m}>{r.scores[m] == null ? <span title={reasonText[r.reasons[m]] || r.reasons[m] || "未完成评分，不作为阴性"}>{missingText(r,m)}</span> : <><strong>{r.common_ranks[m] == null ? "待共同覆盖" : `#${fmt(r.common_ranks[m],1)} / ${r.common_denominator}`}</strong><small>分数 {fmt(r.scores[m],4)}</small>{r.common_percentiles[m] != null && <div className="frontier-meter"><i style={{width:`${100*(1-r.common_percentiles[m]!)}%`}}/></div>}</>}</td>)}<td>{r.disagreement_span==null ? "—" : `${(100*r.disagreement_span).toFixed(1)} pp`}</td></tr>)}</tbody></table></div>}
      {!rows.length && <p>此实体没有进入本次冻结384候选，暂无这三个模型的本轮复核结果。</p>}

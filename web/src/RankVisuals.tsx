...
export default function RankVisuals({ rows, model, view, order, onSort, onSelect, catalogCount }: {
  rows: Ranking[]; model: Model; view: RankView; order: "asc" | "desc"; onSort: (model: Model) => void; onSelect: (row: Ranking) => void; catalogCount: number;
}) {
  const [activeId, setActiveId] = useState<string | null>(null);
  const active = rows.find(row => row.id === activeId);
  const known = rows.filter(row => row.known_relation).length;
  const supported = rows.filter(row => MODELS.filter(m => row.coverage_complete?.[m] && row.ranks[m] != null && row.ranks[m]! <= 20).length >= 3).length;
  return <div className="rank-visuals">
    <div className="rv-summary">
      ...
      <div><span>至少 3 个完整评分模型排入 Top 20</span><strong>{supported}<small> 项</small></strong></div>
      <p><Layers3 size={17} />按 {MODEL_NAMES[model]} {order === "asc" ? "排名升序" : "排名降序"}<br />各模型使用完整目录内已评分数；未齐时为阶段排名</p>
    </div>
    {view === "matrix" ? <div className="rv-matrix-scroll">
      <table className="rv-matrix">
        <caption className="sr-only">当前候选在七个模型中的排名；点击单元格查看配对证据</caption>
        ...
        <tbody>{rows.map(row => <tr key={row.id} onMouseEnter={() => setActiveId(row.id)} onMouseLeave={() => setActiveId(null)} className={activeId === row.id ? "is-active" : ""}>
          ...
          {MODELS.map(m => {
            ...
            const band = rank == null ? "missing" : row.coverage_complete?.[m] === false ? "partial" : rank <= 5 ? "first" : rank <= 20 ? "top" : rank <= den / 4 ? "upper" : "rest";
            return <td key={m}><button className={`rv-cell ${band}`} onClick={() => onSelect(row)} aria-label={`${row.name}，${MODEL_NAMES[m]}，${rank == null ? "暂无评分" : `第 ${rank} 名，共 ${den} 项`}，查看配对证据`}>
              <span><strong>{rank == null ? "—" : `${row.coverage_complete?.[m] === false ? "暂列 " : ""}#${rank}`}</strong><small>{rank == null ? (row.model_status?.[m] === "unavailable" ? "缺少输入特征" : row.model_status?.[m] === "failed" ? "推理失败" : "待评分") : `/ ${den} 已评分`}</small></span>
              <small title={row.model_reasons?.[m]} className="rv-score">分数 {row.scores[m] == null ? "—" : Number(row.scores[m]).toPrecision(4)}</small><i className="rv-cell-meter"><b style={{ width: rank == null ? "0" : `${Math.max(2, (1 - position(rank, den)) * 100)}%` }} /></i>
            </button></td>;
          })}
      ...
      <div className="rv-plot-title"><div><GitCompareArrows size={18} /><strong>同一候选，七种模型视角</strong></div><span>越靠上，模型内排名越靠前</span></div>
      <div className="rv-plot-scroll"><svg viewBox="0 0 1320 335" role="group" aria-label="七模型排名位置平行坐标图">
        {[0, .25, .5, .75, 1].map(p => <g key={p}><line x1="92" x2="1290" y1={55 + p * 225} y2={55 + p * 225} stroke="#d7e1ed" strokeDasharray="3 5" /><text x="76" y={60 + p * 225} textAnchor="end" fill="#526176" fontSize="13">{p === 0 ? "第 1 名" : `${p * 100}%`}</text></g>)}
        {MODELS.map((m, i) => <g key={m} data-model-axis={m}><line x1={140 + i * 180} x2={140 + i * 180} y1="55" y2="280" stroke="#b7c8db" /><text x={140 + i * 180} y="25" textAnchor="middle" fontSize="15" fontWeight="700" fill={MODEL_COLORS[m]}>{MODEL_NAMES[m]}</text><text x={140+i*180} y="44" textAnchor="middle" fontSize="11" fill="#526176">{rows[0]?.denominators[m] ?? 0} / {catalogCount} 已评分{rows[0]?.coverage_complete?.[m] === false ? " · 暂列" : ""}</text></g>)}
        {[...rows].sort((a, b) => Number(a.id === activeId) - Number(b.id === activeId)).map(row => {
          const points = MODELS.map((m, i) => row.ranks[m] == null ? null : { x: 140 + i * 180, y: 55 + position(row.ranks[m]!, row.denominators[m]) * 225, m });
          const focused = activeId === row.id;
          const color = row.known_relation ? "#007d68" : "#2459e8";
          return <g key={row.id} className="rv-trace" role="button" tabIndex={0} aria-label={`${row.name}，查看七模型证据`} onMouseEnter={() => setActiveId(row.id)} onMouseLeave={() => setActiveId(null)} onFocus={() => setActiveId(row.id)} onBlur={() => setActiveId(null)} onClick={() => onSelect(row)} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onSelect(row); } }} opacity={activeId && !focused ? .15 : focused ? 1 : .6}>
        ...
        <text x="680" y="321" textAnchor="middle" fontSize="12" fill="#526176">纵轴 = (排名 − 1) / (该模型评分数 − 1)；评分未齐时为阶段排名；缺失处不连线</text>
      </svg></div>
      <div className="rv-trace-inspector" aria-live="polite">{active ? <><strong>{active.name}</strong>{MODELS.map(m => <span key={m}>{MODEL_NAMES[m]} <b>#{active.ranks[m] ?? "—"}</b></span>)}<button onClick={() => onSelect(active)}>查看证据 <ArrowUpRight size={15} /></button></> : <><CircleHelp size={16} /><span>悬停或用 Tab 选择轨迹，查看候选在七个模型中的排名；点击打开证据。</span></>}</div>
    </div>}

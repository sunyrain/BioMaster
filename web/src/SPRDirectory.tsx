...
import FrontierModels from "./FrontierModels";

export default function SPRDirectory({ initialScope }: { initialScope?: string }) {
  const [scope, setScope] = useState(initialScope === "results" ? "results" : initialScope === "models" ? "models" : "baseline");
  useEffect(() => setScope(initialScope === "results" ? "results" : initialScope === "models" ? "models" : "baseline"), [initialScope]);
  ...
  return <div className="research-browse">
    ...
    <div className="spr-filters sru-directory-tabs" role="group" aria-label="SPR 工作区"><button className={scope === "baseline" ? "active" : ""} onClick={() => setScope("baseline")}>基线384 · 细分实验建议</button><button className={scope === "models" ? "active" : ""} onClick={() => {setScope("models");location.hash="/spr/models";}}>新模型复核 · 分歧</button><button className={scope === "expanded" ? "active" : ""} onClick={() => setScope("expanded")}>新增384 · 证据候选</button><button className={scope === "results" ? "active" : ""} onClick={() => setScope("results")}>实验结果 · 上传与记录</button></div>
    {scope === "models" && <FrontierModels initialView="plot" />}
    ...
    {(scope === "baseline" || scope === "expanded") && <Experiments key={scope} expanded={scope === "expanded"} />}
    {(scope === "baseline" || scope === "expanded") && <section className="panel browse-results">

...
def queue_status(root: Path, job_index: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "allComplete": bool(jobs) and completed_jobs >= len(jobs),
    }
...
def run_command(root: Path, command: list[str], event_name: str) -> int:
    emit(f"{event_name}_starting", command=command)
    code = subprocess.call(command, cwd=root)
    emit(f"{event_name}_finished", command=command, returncode=code)
    return code
...
def run(args: argparse.Namespace) -> int:
    ...
    prerequisite_job_indexes = [resolve(root, item) for item in args.prerequisite_job_index]
    main_score_dir = resolve(root, args.main_score_dir)

    emit(
        "watcher_started",
        mainJobIndex=str(main_job_index),
        prerequisiteJobIndexes=[str(item) for item in prerequisite_job_indexes],
    ...
    last_status = 0.0
    aggregate_job_index = resolve(root, args.aggregate_job_index)
    while True:
        main_status = queue_status(root, main_job_index)
        prerequisite_statuses = [queue_status(root, item) for item in prerequisite_job_indexes]
        score_dirs = [main_score_dir]
        for item in prerequisite_job_indexes:
            score_dirs.extend(score_dirs_from_job_index(root, item))
        ...
        prerequisites_complete = all(item["allComplete"] for item in prerequisite_statuses)
        ready = main_status["allComplete"] and prerequisites_complete and not locks and gpu_idle

        now = time.time()
        if now - last_status >= args.status_every_sec:
            emit(
                "watcher_status",
                mainQueue=main_status,
                prerequisiteQueues=prerequisite_statuses,
                activeLocks=locks,
    ...
    audit_command = [
        sys.executable,
        str(root / "scripts" / "build_diffdock_full_ligand_failure_audit.py"),
        "--root",
        str(root),
    ]
    code = run_command(root, audit_command, "ligand_failure_audit")
    if code != 0:
        return code

    build_command = [
        sys.executable,
        str(root / "scripts" / "build_diffdock_full_multi_ligand_rescue_queue.py"),
        "--root",
        str(root),
        "--max-ligands",
        str(args.max_ligands),
        "--chunk-size",
        str(args.chunk_size),
    ]
    code = run_command(root, build_command, "multi_ligand_queue_build")
    if code != 0:
        return code

    summary_path = aggregate_job_index.parent / "diffdock_multi_ligand_rescue_summary.json"
    try:
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
    except Exception:
        summary = {}
    if int(summary.get("jobs") or 0) <= 0:
        emit("multi_ligand_queue_empty", summaryPath=str(summary_path), summary=summary)
        return 0
    ...
    code = run_command(root, command, "multi_ligand_rescue_queue")
    ...
    for refresh_command in [
        ...
        [sys.executable, str(root / "scripts" / "build_compute_status_dashboard_asset.py"), "--root", str(root)],
    ]:
        run_command(root, refresh_command, "refresh")

    return code


def main() -> int:
    parser = argparse.ArgumentParser(description="Start multi-ligand DiffDock rescue after main and prerequisite rescue queues finish.")

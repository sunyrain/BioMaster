...
def verify(directory):
    ...
    original=build_model(infer_family(state),state['config'])
    ...
    bank=AnchoredFeatureBank(OUTPUT,supplement,SOURCE,packaged.metadata['drug_representation'],local=original.is_local)
...
assert not any("best_model_training" in name for name in sys.modules)

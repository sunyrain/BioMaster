...
1. “非 GPCR 全集”745个靶点均有完整序列，作为不丢信息的登记目录，不直接作为统一rank/745分母。
2. 下游V3主生产范围使用450个有直接小分子证据、且属于五类可建模实验通道的靶点；它与本表中的ChEMBL小分子MoA 450集合数量相同但成员不同。
3. 胞外、非常规和特殊膜体系分到专项路线；无直接小分子证据的特殊体系仅登记保留。缺结构或口袋本身不再删除同一路线内的靶点。
4. 143个GPCR单列为膜蛋白/功能实验分支，不与非GPCR主排名直接混排。
